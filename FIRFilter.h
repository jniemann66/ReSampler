...
public:
	...
	FIRFilter(const FIRFilter& other)
		: length(other.length), currentIndex(other.currentIndex), lastPut(other.lastPut)
	{
		calcPaddedLength();
		allocateBuffers();
		assertAlignment();
	...
	// lazyGet() : Skips stuffed-zeros introduced by interpolation, by only calculating every Lth sample from lastPut
	FloatType lazyGet(int L)
	{
		FloatType output = 0.0;
...
template<typename FloatType>
bool makeLPF(FloatType* filter, int Length, FloatType transitionFreq, FloatType sampleRate)
{
#ifdef FIR_QUAD_PRECISION
...
template<typename FloatType>
FloatType calcKaiserBeta(FloatType dB)
{
	if(dB < 21.0)
	{
		return 0;
...
template<typename FloatType>
bool applyKaiserWindow(FloatType* filter, int Length, double Beta)
{
...
template<typename FloatType>
bool applyKaiserWindow2(FloatType* filter, int Length, double Beta)
{
...
inline std::vector<std::complex<double>> logV(const std::vector<std::complex<double>>& input)
{
...
inline std::vector<std::complex<double>> limitDynRangeV(const std::vector<std::complex<double>>& input, double dynRangeDB)
{
...
inline std::vector<std::complex<double>> realV(const std::vector<std::complex<double>>& input)
{
	std::vector<std::complex<double>> output(input.size(), 0);
	std::transform(input.begin(), input.end(), output.begin(),
...
inline std::vector<std::complex<double>> imagV(const std::vector<std::complex<double>>& input)
{
	std::vector<std::complex<double>> output(input.size(), 0);
	std::transform(input.begin(), input.end(), output.begin(),
...
inline std::vector<std::complex<double>> expV(const std::vector<std::complex<double>>& input)
{
	std::vector<std::complex<double>> output(input.size(), 0);
	std::transform(input.begin(), input.end(), output.begin(),
...
inline std::vector<std::complex<double>> fftV(std::vector<std::complex<double>> input)
{
...
inline std::vector<std::complex<double>> ifftV(std::vector<std::complex<double>> input)
{
	std::vector<std::complex<double>> output(input.size(), 0); // output vector
...
inline std::vector<std::complex<double>> AnalyticSignalV(const std::vector<std::complex<double>>& input)
{
...
template<typename FloatType>
void dumpFilter(const FloatType* Filter, int Length)
{
